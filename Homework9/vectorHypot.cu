 ...
 * Modified by Aaron Santucci for CS 374 Homework 9
 */

/**
 * Vector hypotenuse: a^2 + b^2.
 *
 * This sample is a very basic sample that implements element by element
 * vector hypotenuse. It is the same as the sample illustrating Chapter 2
 * of the programming guide with some hypotenuse like error checking.
 */
...
{
    double stopTime = 0.0;
    double copy1Time = 0.0;
    double arrayTime = 0.0;
    double copy2Time = 0.0;
    ...
    printf("[Vector hypotenuse of %lu elements]\n", numElements);
    ...
    copy1Time = omp_get_wtime() - startTime;
    ...
    arrayTime = omp_get_wtime() - stopTime;
    ...
    copy2Time = omp_get_wtime() - stopTime;

    stopTime = omp_get_wtime();
    // Verify that the result vector is correct
    for (int i = 0; i < numElements; ++i)
    {
        if (fabs(h_A[i]*h_A[i] + h_B[i]*h_B[i] - h_C[i]*h_C[i]) > 1e-5)
        {
    ...
    printf("CUDA total time: %lf\nCUDA copy A&B time: %lf\nCUDA array time: %lf\nCUDA copy C time: %lf\n", stopTime-startTime, copy1Time, arrayTime, copy2Time);
